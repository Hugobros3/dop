...
public class FontRenderer {
	...
	public static void drawText(float xpos, float ypos, float rot, float size,
			String text) {
		drawTextUsingSpecificFontOld(xpos, ypos, rot, size, text, "oldfont");
	}
	...
	public static void drawTextUsingSpecificFontOld(float xpos, float ypos,
		...
		float ratio = 16f*256f/192f;//16f * 12f; // 192
	...
	public static int drawTextUsingSpecificFontHex(float xpos, float ypos,
			float rot, float size, String text, BitmapFont font,String hex,float alpha){
	...
	public static int drawTextUsingSpecificFontRVBA(float xpos, float ypos,
			float rot, float size, String text, BitmapFont font,float alpha,float r,float v,float b) {
		TexturesHandler.bindTexture("./res/textures/font/" + font.name + ".png");
			...
			else
			{
				...
				if(c == '#' && text.length()-i-1 >= 6 && (text.toCharArray()[i+1] != '#') && ColorsTools.isHexOnly(text.substring(i+1, i+7)))
				{
					if(!(i > 1 && text.toCharArray()[i-1] == '#'))
					{
						...
						//System.out.println("colorcode found ! - "+colorCode +" rgb:"+rgb[1]);
						GL11.glColor4f(rgb[0]/255.0f,rgb[1]/255.0f,rgb[2]/255.0f,alpha);
						skip = 6;
					}
				}
				else
